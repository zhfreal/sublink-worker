...
// directionType: -1: reject/block, 0: direct, 1: proxy
export const UNIFIED_RULES = [
    ...
    {
        name: 'Direct',
        outbound: t('outboundNames.Direct'),
        site_rules: ['geolocation-cn', 'cn'],
        ip_rules: ['private', 'cn'],
        directionType: 0
    },
    {
        name: 'Reject',
        outbound: t('outboundNames.Reject'),
        site_rules: ['category-ads-all'],
        ip_rules: [],
        directionType: -1
    },
...
export function getOutbounds(selectedRuleNames) {
    ...
    // PATCH: don't change the order of elements in selectedRuleNames
    return selectedRuleNames.filter(ruleName =>
        UNIFIED_RULES.some(rule => rule.name === ruleName)
    );
...
export function generateRules(selectedRules = [], customRules = []) {
    // patch for null selectedRules, customRules
    if (!selectedRules) selectedRules = [];
    ...
    // PATCH: don't change the order of elements in selectedRules
    selectedRules.forEach(ruleName => {
        const rule = UNIFIED_RULES.find(rule => rule.name === ruleName);
        if (rule) {
            rules.push({
                ...
                outbound: rule.name,
                directionType: rule.directionType
            });
        }
    })
...
// Helper function to get outbound direction types based on single rule
// just return single rule's directionType, its value is 0, 1, or -1
export function getOutboundDirectionType(selectedRuleName) {
    if (!selectedRuleName || selectedRuleName === '') {
        return 1;
    }
...
export function generateRuleSets(selectedRules = [], customRules = []) {
    ...
    //     }
    // });
    selectedRulesSet.forEach(ruleName => {
        const rule = UNIFIED_RULES.find(rule => rule.name === ruleName);
        if (rule) {
            rule.site_rules.forEach(siteRule => siteRuleSets.add(siteRule));
            rule.ip_rules.forEach(ipRule => ipRuleSets.add(ipRule));
...
export function generateClashRuleSets(selectedRules = [], customRules = []) {
    ...
    Array.from(siteRuleSets).forEach(rule => {
        ...
        site_rule_providers[`geosite-${rule}`] = {
    ...
    Array.from(ipRuleSets).forEach(rule => {
        // patch forr rule name: geoip-<rule>
        ip_rule_providers[`geoip-${rule}`] = {
